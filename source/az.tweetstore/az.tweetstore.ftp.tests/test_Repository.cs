...
namespace az.tweetstore.ftp.tests
{
    ...
    public class test_Repository
    {
        ...
        public void Run()
        {
            var sut = new Repository(SERVER, TokenRepository.LoadFrom("ftp.credentials.txt"), "/AppZwitschern/TweetStore");

            Console.WriteLine("storing...");
            var versandaufträge = new[]
                                      {
                                          new Versandauftrag(){Id="1", Termin = new DateTime(2012,3,26, 12, 21, 0), Text = "a"},
                                          new Versandauftrag(){Id="2", Termin = new DateTime(2011,10,27,14, 17, 0), Text = "b"},
                                          null
                                      };
            var versandt = false;
            ...
            Assert.That(results.Select(_ => _==null?null:_.Id).ToArray(), Is.EquivalentTo(new[]{"1","2",null}));
